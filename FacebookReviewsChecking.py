...
def reviewDataInformation(p_name,latestdate):
    ...
    r =  requests.get('https://www.facebook.com/pg/delawareshoutout/reviews/?ref=page_internal')
    ...
    #type(soup)
    for divs in soup:
        divs= soup.findAll('div',class_='_5pcr userContentWrapper')
        print(divs)
...
results=[p for p in f_text if p["reviewSiteid"] == 3] 
for result in results  :
    ...
    url = f'https://surveyedapi.azurewebsites.net/api/Reviews/{result["clientId"]}/3'
    ## ApiKey to access above url 
    my_headers = {'Content-type': 'application/http','ApiKey' : 'rtDe4#434dr5643eDssdr'} 
